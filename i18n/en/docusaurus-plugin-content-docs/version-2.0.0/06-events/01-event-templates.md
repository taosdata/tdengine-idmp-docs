...
An event template defines the schema and behavior of events. Every event generated by an RT analysis must be based on an event template. Templates are managed centrally in **Libraries → Event Template**, making them reusable across any RT analysis in the system.

## 6.1.1 Creating an Event Template

Event templates are managed centrally in Libraries and can be referenced by any RT analysis in the system. To create a new event template:
...
| **Minimum Notification Interval** | The minimum time between consecutive notifications for events from the same RT analysis. Prevents notification overload when an RT analysis fires events frequently — for example, if set to 20 minutes, only one notification is sent per 20-minute window regardless of how many events trigger |
...
| `${Analysis#name}` | Name of the RT analysis that triggered the event |
...
If a template is marked **Base Template Only**, it cannot be referenced directly when configuring an RT analysis — it can only be used as a parent.
...
| **Default Value** | Value used if the RT analysis does not write to this attribute |
...
When configuring an RT analysis, the output of a calculation can be written to any attribute defined in the event template. This supports capturing computed values — like the average value during a window, or the maximum exceedance — directly in the event record.
...
- **Delete** — Remove the template (only possible if no RT analyses reference it)
