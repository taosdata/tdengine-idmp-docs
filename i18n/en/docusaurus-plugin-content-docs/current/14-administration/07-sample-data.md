...
When data for a super table should be loaded from an existing CSV file instead of generated by formulas, add a `csv` block under that super table. CSV mode only changes the data source; `metrics`, `tags`, and `trees` are still defined in the same way.
