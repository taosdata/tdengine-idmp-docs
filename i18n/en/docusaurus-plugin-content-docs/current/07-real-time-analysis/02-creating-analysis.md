...
The Actions section defines actions that run automatically when the analysis fires and a specified condition is met. Each action rule consists of a trigger condition and a referenced [action template](../13-libraries/04-action-templates.md). This section is optional; when no action rules are added, the analysis behaves as before. See [Configuring Actions](./08-actions.md) for full details.
