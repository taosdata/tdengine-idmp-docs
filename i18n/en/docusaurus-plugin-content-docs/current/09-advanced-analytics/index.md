...
**The Analysis Chart is the primary entry point for process analytics.** The Analysis Chart is the only visualization panel in IDMP that operates as an independent window, where users can invoke time-series forecasting, missing data imputation, window analysis, event comparison, correlation analysis, and many other analytical functions — completing an entire analysis workflow within a single workspace. In addition, other panel types such as Trend Charts and Scatter Charts also provide standalone entry points for certain analytical functions in their view pages — Trend Charts support forecasting and imputation, while Scatter Charts support clustering and regression. All advanced analysis features are accessed from the panel **view page**; the edit page is used solely for configuring attributes, metrics, and panel visualization options.
