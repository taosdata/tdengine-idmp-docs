...
| **Symbol Size** | Size of each point (6–40) |
| **Scatter Opacity** | Transparency of each point, 0–1 |
| **Special Point** | Highlight the maximum or minimum of a selected metric with a distinct marker and custom color |
...
| **X Axis** | Show or hide the X axis |
| **X Axis Time Format** | Display format for X axis timestamps (available when X axis is shown) |
| **Rotate Labels** | Rotation angle for X axis labels (-90° to +90°, step 45°) |
| **Label Interval** | Density of X axis labels: Auto, Small, Medium, Large |
| **Show Grid Lines** | Whether to display X axis grid lines: Auto, Show, Hide |
...
| **Value Range** | Minimum and maximum for the left Y axis (leave blank to auto-calculate) |
| **Right Y Axis** | Enable a second independent Y axis on the right, for metrics with very different scales |
| **Right Y Axis Title** | Display name for the right Y axis (available when Right Y Axis is enabled) |
| **Right Y Axis Series** | Metrics assigned to the right Y axis (available when Right Y Axis is enabled) |
| **Value Range** | Minimum and maximum for the right Y axis (available when Right Y Axis is enabled) |

### Limits

Limits overlay reference lines or bands on the scatter plot to mark operating boundaries:

| Setting | Description |
|---|---|
| **Add Limit** | Select a reference metric and add limit types: Maximum, HiHi, Hi, Target, Lo, LoLo, Minimum |
| **Display Config** | Set the value and color for each limit line |
| **Show As** | How to display limits: Line, Area, or Line and Area |

### Transform

The Transform section applies statistical analysis to scatter data, supporting cluster identification and trend fitting:

| Setting | Description |
|---|---|
| **Transform Function Type** | Off (no transform), Data Aggregation (clustering), or Regression Analysis |
| **Method** | Regression method (Regression Analysis only): Linear, Exponential, or Polynomial |
| **Parameters** | Additional configuration: cluster count (for aggregation) or polynomial order (for polynomial regression) |
| **Cluster Config** | Set a name and color for each cluster (Data Aggregation only) |
| **Show Function Label** | Whether to display the regression formula on the chart (Regression Analysis only) |

### Tooltip

| Setting | Description |
|---|---|
| **Tooltip Mode** | Which metrics to show on hover: Single (hovered metric only), All (all metrics), Hidden |
| **Sort Order** | Sort order for multiple metrics in the tooltip: None, Ascending, Descending |
| **Hide Zero Values** | When enabled, metrics with a value of 0 are hidden in the tooltip |
| **Max Width** | Maximum tooltip width in pixels |
| **Max Height** | Maximum tooltip height in pixels |
...
| Setting | Description |
|---|---|
| **Show** | Display mode: List, Table, Hidden |
| **Placement** | Position: Bottom, Right |
| **Width** | Legend panel width in pixels (Right placement only) |
| **Legend Values** | Statistics shown in Table mode (multi-select): Max, Min, Mean, Sum, Count, etc. |
...
| **Max** | Upper bound for values (leave blank to auto-calculate from data) |
| **Decimals** | Number of decimal places to display (leave blank for auto) |
| **Color Scheme** | How series colors are assigned: single color, shades by series, thresholds by value, classic palette, classic palette by series name, custom palette |
...
| **URL** | Target URL, supports variable interpolation |
| **Open in New Tab** | Whether to open the link in a new browser tab |
| **One-click Navigation** | When enabled, clicking a data point navigates directly (only one link per panel can use this) |
...
| **Range** | Match a numeric range |
| **Regex** | Match using a regular expression and replace display text |
| **Special Value** | Match null, NaN, booleans, empty strings, and similar |
| **Other Values** | Catch-all for any value not matched by earlier rules |
...
| Setting | Description |
|---|---|
| **Threshold Mode** | How thresholds are evaluated: Absolute (raw data values) or Percentage (relative to the min–max range) |
| **Add Threshold** | Add a new threshold rule; each rule specifies a numeric boundary and a color |

Color thresholds take effect when the **Color Scheme** in Standard Options is set to **Thresholds (by value)**.

### Overrides

Overrides let you override global graph settings for an individual metric. After selecting a target metric name, you can add override properties such as: series style, line width, fill opacity, line opacity, line color, point size, show points, connect null values, stacking, gradient mode, and show values.

### Downsampling

When query results contain too many data points, enable downsampling to reduce the rendering load and improve display performance:

| Setting | Description |
|---|---|
| **Enable Downsampling** | Toggle, off by default |
| **Max Data Points** | Maximum number of data points to retain after downsampling |
