...
| **URL** | Target URL, supports variable interpolation |
| **Open in New Tab** | Whether to open the link in a new browser tab |
| **One-click Navigation** | When enabled, clicking a cell navigates directly (only one link per panel can use this) |

### Value Mappings

Value mappings define the display color and text label for each state — this is the primary way to configure the appearance of a Status History panel:
...
Color thresholds define value ranges and their associated colors. Combined with value mappings, thresholds can assign gradient colors to continuous numeric values:
