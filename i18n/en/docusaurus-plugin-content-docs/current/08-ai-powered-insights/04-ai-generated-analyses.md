...
AI analysis suggestions appear in the **Analyses** tab of any element in the Explorer. The tab includes an AI panel (visible by default, toggled with the **AI** button in the toolbar) that displays a **Suggested Questions** list — analyses that the system has already generated based on the element's context.
...
**Use a system suggestion.** Click any item in the Suggested Questions list. The AI generates a fully configured analysis form pre-filled with a name, trigger type, calculation expression, and output attribute. Review and click **Save**.
