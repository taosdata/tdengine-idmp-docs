...
![图元进度属性](./images/canvas-progress.gif)
...
![条件触发器](./images/canvas-event-trigger.gif)
...
- `fn`：条件函数，返回布尔值，优先级最高。
- `fnJs`：条件的 JavaScript 代码文本，可获取 `pen` 和 `context` 参数，返回布尔值，优先级次之。
...
![状态](./images/canvas-combine-state.gif)
...
![绑定元素属性](./images/canvas-databind.gif)
