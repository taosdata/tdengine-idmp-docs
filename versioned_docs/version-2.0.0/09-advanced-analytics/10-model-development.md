...
:::note 持续完善中
本模块正在公测中，当前版本已支持时序预测、异常检测与回归等建模场景；聚类、分类等类场景正在开发中，将在后续版本陆续发布，敬请期待。
:::
...
| 模型类型           | 说明                                           | 代表算法                                      |
| ------------------ | ---------------------------------------------- | --------------------------------------------- |
| **时序预测** | 基于历史时序数据预测未来值                     | ARIMA、Prophet等 |
| **异常检测** | 基于无监督学习发现异常值 / 异常曲线 / 异常元素 | iforest-window、iforest-point、Support Vector Machine 等            |
| **聚类**     | 对样本进行无监督群类划分                       | 即将更新                  |
| **分类**     | 以树类算法为核心的样本分类                     | 即将更新     |
| **回归**     | 用多个自变量拟合因变量/输出因变量的估计值      | Liner Regression、Ridge、Lasso、ElasticNet 等       |

:::note
我们正在持续增加更多模型算法与分析场景，敬请期待。
:::

## 9.10.4 使用入口

进入 **管理后台 → 模型** 菜单即可使用，该菜单下包含三个子栏目：

- **模型开发** ：展示训练库内的模型列表，提供机器学习模型的配置与训练功能。
- **模型管理**： 展示模型资产库的模型列表，提供机器学习模型的部署与监控功能。
- **模型数据集**： 展示模型相关的数据集，提供数据集的创建、变量选择与处理功能。
...
模型数据集页面集中展示模型相关的数据集列表，包括数据集名称、状态、采样周期、时间序列数、行数、大小以及创建时间，支持模型数据集的**查看、创建、编辑、导出、删除、变量选择、降采样、插值**等数据操作。

![模型数据集页面](./images/model-10.png)
