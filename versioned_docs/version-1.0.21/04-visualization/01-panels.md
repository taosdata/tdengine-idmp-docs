...
缩略图下方是数据配置区域。区域顶部提供一个切换按钮，用于在**指标配置**和**高级 SQL**之间切换，默认选中**指标配置**。点击该按钮行的空白处，可展开或收起当前配置块。

当系统正在执行查询并刷新预览数据时，当前按钮行右侧会在收起/展开图标左侧显示**正在运行中**提示。

### 4.1.4.6 指标

当切换按钮选中**指标配置**且配置块处于展开状态时，可通过结构化方式定义绘制在图表上的数据系列。标题行提供三个附加控件：
...
当切换按钮选中**高级 SQL**且配置块处于展开状态时，可进入原始 SQL 编辑器。在高级模式下，您可以添加多个 SQL 查询——每个查询显示为一个独立的查询块——所有结果一起显示在同一个面板中。
...
在此面板上配置定时报告发送规则。详情请参阅[定时报告](./06-scheduled-reports.md)。

其他设置部分——图形、坐标轴、边界值、图例等——是特定于面板类型的，在各面板类型的章节中进行说明。

## 4.1.5 注解

注解让用户为面板附加文字备注——例如标记异常原因、记录维护操作或为团队成员留下上下文说明。

### 添加注解

1. 在面板查看模式中，点击工具栏的 **注解** 按钮打开注解面板。
2. 点击底部的 **+** 按钮展开输入框。
3. 输入注解内容（支持 Markdown 格式）。
4. 按 **Enter** 提交，按 **Shift+Enter** 换行。

### 编辑注解

将鼠标悬停在注解上，点击 **编辑** 图标。修改内容后点击 **保存** 按钮保存更改，或点击 **取消** 放弃修改。

### 删除注解

将鼠标悬停在注解上，点击 **删除** 图标，在确认对话框中确认后删除。

## 4.1.6 面板管理

**分类** 是在通用设置中分配给面板的自由文本标签。它们显示在面板选项卡的"分类"过滤下拉菜单中，让用户按功能或系统区域快速查找面板（如电气、机械、质量）。

**分组** 将面板加入自定义命名分组以便快速访问。已加入分组的面板可通过全局仪表板视图左侧边栏中的**面板分组**直接访问。
