...
在**分析工作台**（Analysis Workbench）的查看模式下，点击操作栏中的**相关分析**图标即可使用相关分析功能。只有已添加到分析工作台中的时序属性才能参与相关分析。

步骤：

1. 打开或创建一个**分析工作台**，添加需要分析的时序属性（至少两个）。
...
| **属性列表** | 显示当前分析工作台中已添加的所有属性，用户可选择哪些属性参与分析 |
...
1. 打开分析工作台，添加 `塔底纯度`、`回流比`、`进料温度`、`塔顶压力`、`再沸器热负荷` 五个属性，时间范围设为过去 7 天。
