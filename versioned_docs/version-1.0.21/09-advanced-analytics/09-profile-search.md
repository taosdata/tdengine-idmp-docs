...
相似度分析用于在历史时序数据中搜索与用户指定的目标波形最相似的时间片段。用户在分析工作台中框选某个属性的一段时间窗口作为参考模式，系统在整个可见时间范围内滑动扫描，计算每个候选片段与参考模式之间的相似度，并将最匹配的结果以高亮窗口的形式叠加展示在图表上。
...
1. 用户在分析工作台中用鼠标框选某个属性的一段时间范围，该区域即为**初始窗口**。
2. 系统在当前分析工作台的整个时间范围内，以滑动窗口方式生成一系列候选窗口。
...
在**分析工作台**（Analysis Workbench）的查看模式下，点击操作栏中的**相似度分析**按钮即可使用。

### 操作步骤

1. 打开或创建一个**分析工作台**，添加需要分析的元素属性。
...
6. 分析完成后，初始窗口与找到的相似窗口在分析工作台高亮显示
...
| **窗口颜色** | 指定初始窗口与目标窗口在分析工作台中的高亮背景颜色 |

![相似度配置](./images/profile-search-02.png)

点击**确定**后，系统开始执行窗口搜索与相似度计算。相似度计算涉及滑动窗口下的多轮迭代，执行需要一定时间。任务执行期间该分析工作台不可执行其他分析操作。后台执行过程中用户可随时取消当前任务。

### 结果展示

相似度分析完成后，结果以两种形式呈现：

**窗口高亮：** 在分析工作台中，初始窗口和相似度分析找到的目标窗口将叠加显示在分析工作台中。初始窗口与相似窗口均使用上一步用户定义的背景颜色高亮区别显示。此时，用户可以调用分析工作台的事件分析功能，对这些窗口事件进行深入分析，如事件线、开始时间对齐、归一化等。

![相似度显示](./images/profile-search-03.png)

**结果表格：** 点击分析工作台操作栏的**事件、属性列表**图标，将弹框展示所有相似窗口的详细信息，相似窗口将作为一类事件展示，并按窗口相似度从高到低排序。弹框窗口右上角提供**导出**按钮，可以将当前列表结果导出为 CSV 文件。
...
:::note
初始窗口和相似度分析找到的目标窗口在分析工作台中均被视为一种自定义事件，这些窗口均可以与其他系统事件一起，调用分析工作台中的事件分析能力进行深入分析，这极大方便了用户的探索过程。
:::
...
1. 在分析工作台中打开注塑机注射压力趋势数据，时间范围设为近 30 天。
