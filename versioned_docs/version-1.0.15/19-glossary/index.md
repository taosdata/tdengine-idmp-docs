...
专门用于采集、存储和检索工业过程时序数据的软件系统。工业实时数据库位于现场系统（PLC、SCADA、DCS）与上层分析或报表工具之间，提供运营数据的长期记录。部署最广泛的工业实时数据库是 AVEVA PI System。TDengine Historian——结合 TDengine TSDB 的存储能力和 TDengine IDMP 的资产建模与分析能力——是传统数据史学家的现代替代方案，增加了 AI 原生能力、开放接口，并以更低成本提供显著更高的扩展性。
