...
数据目录让 AI 与用户都能在无需了解底层数据库表名或原始数据模式的情况下，*定位、理解并导航*数据资产。用户或 AI 智能体无需追问"TDengine 中哪张超级表存储了站点 A 三号泵的振动数据？"，只需导航至 `站点A → 公用工程 → 三号泵 → 振动`，即可立即找到所需数据，连同其工程单位、历史趋势、实时分析规则和关联文档一并获取。
...
我们以风力发电厂为例。一台风机或一台逆变器，可以同时出现在地理层级树和功能设备层级树中。两个视图指向同一个元素及其数据，不存在任何重复。这是通过*元素引用*实现的。详情参见 [3.1.7 元素引用](#317-元素引用)。
...
| **分析模板** | 在该类型每个元素上运行的可复用实时分析规则。参见[分析模板](../07-real-time-analysis/07-analysis-templates.md)。 |
...
上图中，风机-1 在"风机"和"站点 A"下均有强引用。从"站点 A"下删除风机-1，只是移除了该引用——风机-1 仍然存在于"风机"下。
