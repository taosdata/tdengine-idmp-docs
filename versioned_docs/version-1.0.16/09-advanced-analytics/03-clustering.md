...
第一种，通过**散点图面板**在查看模式下的操作栏中的**聚类分析**图标进行访问。
...
第二种，通过**分析面板**操作栏中的**启用 XY Plot** 图标进行访问。

步骤：

1. 打开或创建一个**分析面板**，点击**启用 XY Plot** 图标，在弹出框内选择 X 轴与 Y 轴属性，点击应用，此时面板将变成散点图形式。

![分析面板中的聚类分析](./images/scatter-clustering-2.png)

2. 点击操作栏的**启用框选**图标，此时可以用鼠标在散点图内框选数据范围，系统将基于选中的数值点自动生成新的窗口分析。

![分析面板中的聚类分析](./images/scatter-clustering-3.png)

3. 点击操作栏的**聚类分析**图标，在弹出框内确定聚类分析配置后，系统将生成聚类分析结果，并在散点图中用不同颜色显示。

![分析面板中的聚类分析](./images/scatter-clustering-4.png)

:::note
当前版本中，聚类分析的使用入口为分析面板与散点图面板查看模式操作栏中的聚类分析图标。未来版本将继续扩展算法类型和使用方式。
