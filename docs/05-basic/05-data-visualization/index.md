...
3. **计量单位 (UOM)**：设置指标的显示单位，可调整为同一计量单位分类下的其他单位。
...
4. `${Element#name}`: 在面板或者仪表板中使用，用于替换当前元素的名称
...
4. 参数 `${Element#name}` 会被替换为一个字符串，使用参数时，需要添加单引号
...
面板支持分享功能。在面板查看页面，点击右上角的分享图标，设置分享时长，可以生成一个唯一的分享链接。通过该链接，其他用户可以访问并查看该面板，也可以嵌入第三方应用页面。
