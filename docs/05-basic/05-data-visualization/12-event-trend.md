...
默认不开启。开启“按开始时间对齐”后，可开启。开启后会将不同事件的时长按照平均值进行归一化处理，原来的采集时长被调整，使得不同时长事件的数据可以在同一时间维度中进行对比分析。

![归一化](./images/event-trend-normalization.png)

### 包络线

默认不开启。打开启“归一化”后，可开启。开启后，会在事件趋势图中显示包络线，用于展示指标的波动范围和趋势。

如图，首先设置包络线范围，确认后，即可显示包络线。0 表示最小值，100 表示最大值。
![包络线设置](./images/event-trend-envelop-setting.png)

![包络线展示](./images/event-trend-envelop.png)
