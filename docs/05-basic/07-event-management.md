...
打开事件详情页面，点击右上角的“根因分析”按钮，开始生成根因分析。AI 将基于当前事件的关键数据，生成一份结构化的、易于理解的、具有业务洞察力的专属分析报告，帮助用户快速排查事件产生的根本原因，降低运维成本。
...
对于任何一个元素，所有的事件共享一个通知规则。规则里可以指定联系途径 (Contact Point)，重发间隔，升级联系途径，升级时间间隔和最大重发次数。消息体是一个文本文件，但系统已经预置了信息，并设置了替换字符串。

为简化管理，子元素创建时，永远继承父元素的通知规则，但您可以修改。修改后，该子元素以及该子元素的子元素将采用新的通知规则发送消息。

### 事件模板

事件一定有模板，是否发送通知，取决于该事件模板严重级别的设置。只有严重级别达到或超过设置的级别，才会发送通知消息。即使属于同一个事件模板，不同事件可以有不同的严重级别。事件模板中可以设置事件间通知最小发送间隔，只有距分析的上一次通知时间大于此间隔时，事件才会发送新的通知。这样多种参数设置，可以避免消息过多。
