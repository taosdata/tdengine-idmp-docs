...
| **缺失值填补**           | 进入填补模式，在图表上拖拽选取数据缺口区域，系统将使用 AI 趋势估算自动填充                               |
| **重置填补**       | 移除当前图表上已应用的所有填补结果  |
| **显示注解** | 在页面右侧展示当前图标的注解信息图      |
| **AI 解读**       | 对当前图表运行 AI 面板解读或通用分析，输出文字分析报告，也可以添加并执行个性化的 AI Function             |
...
阈值设置根据数据值动态改变系列的颜色，用于高亮显示超出正常范围的数据段：
