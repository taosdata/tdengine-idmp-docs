...
| 职责 | 说明 |
|---|---|
| **统一入口** | 对外暴露单一地址/端口，后端服务不直接暴露 |
| **路由与负载均衡** | 将请求分发至 IDMP 和 TSDB 实例 |
| **安全** | TLS 终止、统一认证（Token/SSO）、IP 访问控制 |
| **流量治理** | 限流、熔断、重试、超时、灰度发布 |
| **可观测性** | 集中访问日志、指标和分布式追踪 |

### 外部依赖

| 组件 | 作用 | 单实例替代方案 |
|---|---|---|
| **Redis** | 缓存、短生命周期状态、分布式锁 | 内部缓存与锁 |
| **MySQL** | 关系型元数据（用户、权限、任务、配置） | 内嵌 H2 数据库 |
| **分布式文件系统** | 文件/对象持久化（元数据、图形、导入/导出文件） | 本地文件系统 |
| **Elasticsearch** | 集中式索引管理和全文检索 | 内部 Lucene |
| **Kafka** | 异步消息与事件总线（解耦、任务编排、通知） | 内部消息队列（单实例）或 Redis MQ（最小高可用） |
| **Apollo** | 配置中心（动态配置、版本管理） | 内部服务配置 |
