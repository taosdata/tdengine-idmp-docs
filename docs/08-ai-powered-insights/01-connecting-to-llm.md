...
IDMP 中的大多数 AI 功能——面板生成、实时分析建议、AI 问答、根因分析——都需要连接外部大语言模型（LLM）。IDMP 使用 OpenAI 兼容接口，因此任何提供 OpenAI 兼容 API 的大语言模型服务商或自托管模型均可使用。
...
- **问答模型** — 处理日常交互：回答自然语言查询、生成面板建议、创建实时分析配置以及叙述面板洞察。该模型应具备速度快、成本低的特点。
...
> **注意**：对于自托管大模型，为确保本产品 AI 相关功能正常运行，建议模型能力在 **Qwen/Qwen3.5-27B** 及以上级别。
> 
> 以下是推荐的参考模型列表（按推荐优先级从高到低排序）：
> - `qwen3.5-plus`
> - `glm-5.2`
> - `kimi-k2.6`
> - `deepseek-v4-pro`
> - `Qwen/Qwen3.5-27B`（**建议配置**）
> - `gpt-oss-120b`
> - `gemma-4-31b-it`
