...
AI Agent 支持通过即时通讯工具与您交互。配置消息通道后，您可以在飞书、Telegram、微信、钉钉、Slack 或 WhatsApp 上与 AI Agent 对话，而无需始终打开 IDMP Web 界面。系统管理员配置通道后，每个用户需将自己的 IM 账号与 IDMP 用户账号绑定，以建立安全的对话通道。
...
| **WhatsApp** | 系统开关 + 回复前缀 Emoji | 二维码扫码（与微信类似）               | 海外用户常用，基于 WhatsApp Web 多设备协议，无需 Business API |
...
WhatsApp 渠道基于 **whatsmeow** 库（WhatsApp Web 多设备协议）实现，通过 二维码扫码完成设备配对。与飞书 WebSocket、Telegram long-poll 属同类连接模型。

> ⚠️ **重要提示**：
>
> - WhatsApp Web 协议为非官方 API，有被封号风险，**建议使用独立号码**
> - WhatsApp 限制每个号码最多关联 4 台设备，IDMP 占用 1 个关联设备位
> - 24 小时会话窗口：用户发起消息后 24h 内可自由回复，超时需用户重新发消息
> - 当前版本仅支持文本消息，不支持图片/文档/语音/群组消息

**配置步骤：**

1. 在通道管理列表中找到 **WhatsApp** 行，点击「详情」进入配置页。
2. 点击**编辑**按钮。
3. 配置**回复前缀 Emoji**（默认 🤖，Agent 回复前显示的标识，用于区分 Agent 回复和用户消息）。
4. 开启**启用**开关。
5. 点击**保存**。
...
| 格式 | 处理方式 |
|------|---------|
| Markdown 表格 | 自动转换为 `cell \| cell` 文本格式 |
| 面板链接 | 渲染为 `📊 Panel #ID\nURL`（WhatsApp 自动识别可点击链接） |
| 建议问题 | 编号列表格式 |
| 超长消息 | 自动拆分为多条消息（每条 ≤4000 字符） |
...
在个人设置页的 **IM 绑定**区域，始终显示六个通道的状态卡片，无论您是否已绑定：
...
WhatsApp 绑定流程与微信类似，通过 二维码扫码完成，无需输入绑定码。

1. 确保管理员已启用 WhatsApp 通道。
2. 点击页面右上角个人头像，在 IM 绑定区域，WhatsApp 行点击**绑定用户**。
3. 系统显示 二维码（从 WhatsApp 获取）。
4. 打开手机 WhatsApp → **设置** → **已关联设备** → **关联设备**。
5. 扫描屏幕上的 二维码。
6. 绑定成功后，系统自动启动消息监听，状态更新为「已绑定」。

> ⚠️ **二维码有效期约 20 秒**，过期后点击「重新获取二维码」获取新的 二维码。

**自聊天模式：** WhatsApp 支持自聊天（向自己发消息）。Agent 回复前会自动添加 emoji 前缀（默认 🤖），以便区分 Agent 回复和用户自己的消息。该前缀可在 Admin Console 的 WhatsApp 配置中修改。
...
**Q：WhatsApp 二维码扫码后显示"无效二维码"？**

A：二维码有效期约 20 秒。点击「重新获取二维码」获取新的 二维码后立即扫码。

**Q：WhatsApp 绑定后收不到回复？**

A：请检查以下几点：1) 管理员是否已在 Admin Console 启用 WhatsApp 渠道；2) 手机 WhatsApp 是否正常连接网络；3) 手机端是否已注销了关联设备（如已注销需重新扫码绑定）。

**Q：WhatsApp 连接频繁断开？**

A：可能原因包括网络不稳定、手机 WhatsApp 版本过旧或 WhatsApp 服务器维护。系统支持自动重连，断开后会自动尝试恢复连接。

**Q：WhatsApp 支持群组消息或图片/文档吗？**

A：当前版本（v1）仅支持单聊文本消息，不支持群组消息、图片、文档或语音消息。
