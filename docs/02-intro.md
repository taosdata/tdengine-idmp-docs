...
3. TDengine IDMP 不提供企业管理所需的人员、排班、维修工单、质量、仓储等功能，但支持通过 SDK 与这些系统实现无缝集成，或直接对接相关系统的数据库获取元数据。

TDengine 只是一物联网、工业数据平台，能高效的采集、存储数据，而且通过简单易用的方式，帮助用户分析数据，获得实时的业务洞察，便于做出商业决策，但它不是企业执行系统，更不是控制系统。
