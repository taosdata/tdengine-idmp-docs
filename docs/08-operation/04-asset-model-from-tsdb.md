...
9. 属性模板名称可以为空，若属性模板为空，该超级表列会自动映射到同名的属性模板，若属性模板不存在则会自动创建。若设置了属性模板名称但是属性模板不存在也会自动创建。若属性模板名称包含 `Quality:属性模板名称A` 则表示该属性是某个 A 这个属性模板的质量列。
...
1. 数据以单列模型存储。
2. 超级表需要有一列表示当前子表所属的元素及属性信息。示例：子表路径列的值是 `Site1.tank02.temperature`，及表示该子表存储的数据是元素 `Site1` 下的元素 `tank02` 的属性 `temperature`，假如另外一个子表路径列的标签值是 `Site1.tank02.pressure` 那么该子表对应元素也是 `tank02`，但是属性是 `pressure`。
...
5. 在选择了数据库之后，下方表格会加载该数据库下的超级表信息，默认勾选所有的超级表，表示处理所有的超级表数据，如果要忽略某个超级表下的子表取消勾选该超级表即可。每张超级表需要配置其路径列、数据列和质量列。其中路径列表示该超级表下的子表中的哪个标签列的值作为该子表对应元素和属性的解析依据，数据列则是生成的属性其数据引用设置对应得列，质量列则是该属性数据引用的质量引用列。
6. 超级表也可以配置`路径开始层级`，表示子表路径列的值在处理的时候忽略多少层级，比如超级表配置路径开始层级是 1，若子表路径列的标签值是 `Site1.tank02.temperature`，那么在处理的时候会直接从 `tank02.temperature` 开始处理，会忽略 `Site1` 的存在。若子表路径列的值在经过路径开始层级处理之后其长度不足 2，那么会忽略该子表的处理，比如 `Site1.tank02.temperature` （该长度可以认为是 3，及经过 `.` 分隔之后可以得到 3 级）若设置的开始层级为 2，剩余的路径是 `temperature` 长度不足 2，那么该子表就会忽略不再处理。
