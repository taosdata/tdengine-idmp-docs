# 从 TDengine TSDB 创建数据模型

对于已经在使用 TDengine TSDB 的用户，IDMP 支持从 TDengine TSDB 创建数据模型。它提供三种方式，下面一一介绍。
...
## 超级表映射为元素（Map STable to Element）

如果数据已经写入 TSDB，但并没有很好的建模，比如没有树状结构的标签，虽然一个设备有多个采集量，但采用的是单列模型等。这个时候，就不能使用简单导入方式创建数据模型了。IDMP 提供了一种灵活但需要复杂配置的方式，让您将多个超级表汇聚到一个元素模板，并通过字符串生成器（string builder）、命名模式（naming pattern）等构建出树状结构。它能完美地解决单列模型建模的问题，让 OT 域的数据（比如 PLC 采集的数据）在 IT 域得以更好的结构化呈现，具有业务语义，便于获取业务洞察。
...
整个创建流程是选择数据库、超级表，然后选择元素模板，配置元素名称与元素路径，然后将超级表里的标签与时序数据映射到元素模板的属性。

具体步骤如下：

1. 管理后台，创建到 TDengine TSDB 的连接。
2. 在连接的列表页，或左侧树状结构的列表，点击三个点菜单，选择`超级表映射为元素`（Map STable to Element）。
3. 页面的上方下拉框里将显示该连接里的数据库，选择一个数据库后，右侧的下拉框将显示该库拥有的超级表。
4. 选择元素模板
...
10. 点击`完成`，系统就会创建数据模型任务。

上述步骤只是完成了一个超级表，甚至只是一个超级表的部分指标的映射配置工作。对于完整的数据模型创建，需要配置多个资产模型。极端情况，对于单列模型，如果每个设备类型有 100 个属性，有 10 个不同类型的设备，那么就要创建 100 * 10 个资产模型。这个工作量是很大的，需要考虑使用从 CSV 创建数据模型功能。

**自动同步**：上述资产模型创建后，如果 TSDB 里新增了子表，它将自动同步到 IDMP，不用人的干预。

**新增超级表**：资产模型创建后，如果 TSDB 里新增了超级表，需要添加新的资产模型，否则新的超级表里的元数据无法同步到 IDMP，新的数据资产无法在 IDMP 里呈现。

## 从 CSV 导入（Import from CSV）

在使用了超级表映射为元素（Map STable to Element）导入之后会很快发现，若单列模型下超级表数量过多的话，任务创建将变得十分繁琐。虽然现在已经支持了元素模板由 CSV 文件进行创建但是属性模板和超级表的映射还是需要多次创建任务。IDMP 提供了基于 CSV 文件的数据导入，此种方式可以认为是`简单导入`和`超级表映射为元素`的批量操作。使用 CSV 数据导入，可以提前创建好元素模板，也可以不创建元素模板。
...
8. 超级表列名不能为空。元素模板分类支持设置以英文逗号分隔的字符串，若分类不存在则会自动创建。属性模板分类同理。
9. 属性模板名称可以为空，若属性模板为空，该超级表列会自动映射到同名的属性模板，若属性模板不存在则会自动创建。若设置了属性模板名称但是属性模板不存在也会自动创建。若属性模板名称以 `Quality:` 为前缀，则其后紧跟的字符串 `<属性模板名称>` 必须与被关联的属性模板名称完全相同，表示“当前这一列是 `<属性模板名称>` 这个属性模板的质量列”，例如：要将某一列配置为属性模板`温度`的质量列，则在该列的“属性模板名称”中填写 `Quality:温度`。
...
CSV 配置文件使用`UTF-8`编码格式，暂时不支持`UTF-8 with BOM`编码。在使用中文版 Windows 系统并且使用 Excel 软件打开的时候可能会显示乱码，建议使用 WPS 或者 Notepad++ 等其他工具中进行编辑。如果只能使用 Excel 软件编辑请注意该方式保存的 CSV 文件编码可能是 `UTF-8 with BOM`，需要转换为`UTF-8`编码的文件才能正常上传。

:::

**CSV 配置文件示例：**

1. 元素路径在超级表中已经存在定义。这里以 TDengine TSDB 常用的智能电表做示例，在智能电表的超级表（meters）结构中，定义了`location`的标签，该标签中存储了以 `.` 分隔的路径，可以认为该元素路径在超级表中已经存在。meters 的子表每个子表对应一个元素，由于没有元素名称的标签，所以定义使用子表名作为元素名。那么此种情况下的 CSV 配置示例如下：

    <!-- autocorrect-disable -->
    <div className="table-nowrap">

    | 数据库名称 | 超级表名称 | 元素模板名称 | 元素模板分类 | 子表名称 | 子表过滤规则 | 元素名称表达式 | 元素描述表达式 | 元素路径表达式   | 超级表列名 | 属性模板名称 | 引用类型        | 属性模板分类 | 属性模板描述 | 属性模板是否隐藏 | 属性模板是否排除 | 属性模板默认计量单位 | 属性模板显示计量单位 | 属性模板默认值 | 属性模板显示数字位数 |
    |------------|------------|--------------|--------------|----------|--------------|----------------|------------------|------------------|------------|--------------|-----------------|--------------|--------------|------------------|------------------|----------------------|----------------------|----------------|----------------------|
...
2. 元素路径是需要通过多个标签值组合出来的情况。此种情况不同于示例 1 中的 `location`标签的值已经是`.`分隔的字符串。元素路径需要由多个标签组合起来，形成类似`${tag1}.${tag2}.${tag3}`的组合。以超级表`vm_diskio`为例，该表存在`location`、`rack_num`、`slot_num`，在定义元素路径的时候需要三者进行组合，同时使用`vm_host`作为元素名称，元素模板定义为`虚拟机磁盘IO`，该模板目前在基础库中并不存在，会依据 CSV 中的配置自动创建出来。CSV 的配置示例如下：
...
3. 子表中完全没有路径相关信息的情况。此种情况有两种处理方式，一种是表结构支持修改，为表添加一个新的标签列用于存储该子表对应的元素路径，当经过此种方式修改之后可以通过上述两种方式指定元素路径；另外一种处理方式即需要在 CSV 显示配置元素路径，接下来使用一个单列模式的存储结构展示此种情况下的配置方式。超级表名为`opc_stw`，该超级表下的子表分别对应四个设备`进出水监测`、`生化池北组`、`生化池南组`、`MBR膜池`。其中`生化池北组`、`生化池南组`共同使用`生化池`这个元素模板。
...
## 从 OPC 导入（Import From OPC）

对于采集的 OPC UA 的数据来说，通常在 TDengine TSDB 中以单列模型进行存储。如果使用上面的配置方式将会异常复杂。在单列模型下，一个设备的点位数据可能会存在于一个或多个超级表下，这样就要求在 IDMP 构建数据模型的时候能够将所有属于同一个设备的子表关联到一起。在使用“从 OPC 导入”功能下，可以非常简单的实现。
由上得知，使用“从 OPC 导入”这个功能对数据是有要求。具体来说有以下要求：

1. 数据以单列模型存储。
2. 超级表需要有一列表示当前子表所属的元素及属性信息。示例：子表路径列的值是 `Site1.tank02.temperature`，即表示该子表存储的数据是元素 `Site1` 下的元素 `tank02` 的属性 `temperature`，假如另外一个子表路径列的标签值是 `Site1.tank02.pressure` 那么该子表对应元素也是 `tank02`，但是属性是 `pressure`。

具体步骤如下：

1. 管理后台，创建到 TDengine TSDB 的连接。
2. 在连接的列表页，点击 TSDB 连接，之后点击 `从 OPC 导入`。
3. 页面的上方下拉框里将显示该连接里的数据库，选择要处理的数据库后，每次提交任务可以提交多个数据库的配置，注意取消勾选忽略，默认所有的库忽略处理。
4. 在选择数据库之后，可以选择`父元素`，所谓父元素表示在使用该数据库下的子表数据构建元素树的时候会以该选择的父元素作为该子树的根结点。
5. 在选择了数据库之后，下方表格会加载该数据库下的超级表信息，默认勾选所有的超级表，表示处理所有的超级表数据，如果要忽略某个超级表下的子表取消勾选该超级表即可。每张超级表需要配置其路径列、数据列和质量列。其中路径列表示该超级表下的子表中的哪个标签列的值作为该子表对应元素和属性的解析依据，数据列则是生成的属性其数据引用设置对应的列，质量列则是该属性数据引用的质量引用列。
6. 超级表也可以配置`路径开始层级`，表示子表路径列的值在处理的时候忽略多少层级，比如超级表配置路径开始层级是 1，若子表路径列的标签值是 `Site1.tank02.temperature`，那么在处理的时候会直接从 `tank02.temperature` 开始处理，会忽略 `Site1` 的存在。若子表路径列的值在经过路径开始层级处理之后其长度不足 2，那么会忽略该子表的处理，比如 `Site1.tank02.temperature` （该长度可以认为是 3，即经过 `.` 分隔之后可以得到 3 级）若设置的开始层级为 2，剩余的路径是 `temperature` 长度不足 2，那么该子表就会忽略不再处理。
7. 点击`完成`，系统就会创建数据模型任务。

:::note

目前通过`从 OPC 导入`生成的元素都是没有元素模板的，这对于后期对于同类设备创建分析、面板等会变得繁琐。所以在导入任务创建完元素之后可以在元素列表页面可以将同类元素添加到同一个元素模板。具体可以通过在`元素浏览器`下点击搜索，输入要搜索的条件，在返回的元素列表页面，勾选多个元素，并选择`添加到模板`，将勾选的元素添加到同一个元素模板，方便之后创建分析和面板。
