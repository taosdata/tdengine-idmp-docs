...
2. 单击右上角头像，打开个人信息弹窗。
3. 在 **登录令牌** 区域复制当前令牌。
4. 将复制到的完整值原样写入 `Authorization` Header，例如 `Bearer <IDMP_TOKEN>`。
...
| 项目 | 说明 |
|---|---|
| 推荐访问地址 | `https://idmp.tdengine.net:6034` |
...
| 默认 HTTPS 端口 | `6034` |
| HTTP 排障地址 | `http://idmp.tdengine.net:6042/api/v1/mcp/stream` |

将示例中的 `https://idmp.tdengine.net:6034` 替换为您的实际 IDMP HTTPS 地址。生产环境建议优先使用 HTTPS；仅在证书或网络排障时，才临时切换到 HTTP 地址。
...
如果希望将配置共享给当前项目，可额外加上 `--scope project`。如果当前环境只能使用 SSE，请将 URL 替换为 `https://idmp.tdengine.net:6034/api/v1/mcp/sse`，并按客户端要求选择对应的 transport 类型。
...
启动 Codex 前，请先在当前终端设置 `IDMP_TOKEN`。如果当前环境需要 SSE，可将 URL 改为 `https://idmp.tdengine.net:6034/api/v1/mcp/sse`；如客户端版本需要显式 transport 字段，请设置为 `sse`。
...
如果当前环境需要 SSE，请将 URL 改为 `https://idmp.tdengine.net:6034/api/v1/mcp/sse`，并在界面中选择 `SSE` 或等价的 transport 类型。
...
请先确认域名解析、证书链和客户端信任链是否正确。如果只是临时排查连通性问题，可以先改用 `http://idmp.tdengine.net:6042/api/v1/mcp/stream` 验证网络是否可达，排障完成后再切回 HTTPS。
