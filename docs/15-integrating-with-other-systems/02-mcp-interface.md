...
将示例中的 `<IDMP_HOST>` 替换为您的实际 IDMP 域名或 IP。生产环境建议优先使用 HTTPS；仅在证书或网络排障时，才临时切换到 HTTP 地址。
...
启动 Codex 前，请先在当前终端设置 `IDMP_BEARER_TOKEN`，并确保其值**不包含** `Bearer` 前缀。
...
将示例中的 `<IDMP_HOST>` 替换为您的实际 IDMP 域名或 IP。生产环境建议优先使用 HTTPS；如需排查证书或网络问题，请保留 `/api/v1/mcp/sse` 路径，仅将协议和端口临时切换为 HTTP 和 `6042`。
