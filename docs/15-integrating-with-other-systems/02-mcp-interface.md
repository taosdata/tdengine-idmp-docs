...
2. 单击右上角头像，打开个人信息弹窗。
3. 在 **登录令牌** 区域复制当前令牌。
4. **登录令牌** 区域显示的是完整的 `Authorization` Header 值，已包含 `Bearer` 前缀。对于需要 `Authorization` Header 的客户端，可直接原样粘贴，例如 `Authorization: Bearer <token>`。
...
将示例中的 `<IDMP_HOST>` 替换为您的实际 IDMP 域名或 IP。`<IDMP_LOGIN_TOKEN>` 表示从界面复制的完整登录令牌值，已包含 `Bearer` 前缀。生产环境建议优先使用 HTTPS；仅在证书或网络排障时，才临时切换到 HTTP 地址。
...
下文中，`<IDMP_LOGIN_TOKEN>` 表示从界面复制的完整登录令牌值，已包含 `Bearer` 前缀；`<IDMP_BEARER_TOKEN>` 表示去掉 `Bearer` 前缀后的纯 token 值。
