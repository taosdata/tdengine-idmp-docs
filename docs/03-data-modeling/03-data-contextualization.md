...
IDMP 平台将这些属性上下文信息作为关键数据对象来集中管理，实现这些上下文信息的统一定义、全局引用、动态更新。用户可以在元素模板里配置上下文信息，用户也可以为特定设备其配置不一样的上下文信息。不管哪种配置方式，只需配置一次，就全局可用。这些上下文一旦被配置就自动成为 IDMP 平台中可被广泛引用的数据对象——可视化面板上的参考线、实时分析中的触发条件、属性表达式中的计算变量，全部指向同一个定义。极限值管理相关概念可以查阅 [限值管理](https://www.taosdata.com/tdengine-engineering/36580.html)。
