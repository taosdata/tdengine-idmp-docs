...
```text
CONCAT('Device', CAST(${attributes['Device ID']} AS varchar), ' voltage is ', CAST(${attributes['Voltage']} AS varchar), 'V')
```
...
| **显示精度** | 显示值时保留的小数位数 |
| **数据引用类型** | 属性值的来源：TDengine 指标、TDengine 标签或无（参见 [3.2.2](#322-属性类型)） |
