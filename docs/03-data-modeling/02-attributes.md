...
:::note 命名规则
属性名称必须满足以下规则（属性与属性模板遵循相同的规则）：

- 名称不能为空，长度不能超过 255 个字符。
- 名称不能包含特殊字符 `$`、`{`、`}`（这些字符与替换参数 `${...}` 的语法冲突）。
- 名称在同一元素（或同一元素模板）内必须唯一；同一父属性下的子属性、特性的名称也必须唯一。

不满足规则时无法保存，系统会提示相应错误。此外，描述的长度不能超过 2048 个字符。
:::
...
| 字段 | 说明 |
|---|---|
| **名称** | 属性名称，命名规则与属性相同（参见 [3.2.3.1 节](#3231-基础字段)的命名规则说明） |
| **描述** | 可选描述 |
| **配置** | 附加配置标志（如隐藏、常量） |
| **类别** | 类别标签 |
