...
基于描述系统运行规律的物理定律——质量守恒、能量守恒、热力学方程、流体动力学、反应动力学或结构力学——而构建的数学模型，而非单纯从观测数据中拟合得出。也称为第一性原理模型或白盒模型。由于机理模型将过程实际运行原理以领域知识的形式编码其中，它能够外推到历史数据中未见过的工况，并提供工程师可依据已知物理规律进行验证的可解释结果。

机理模型在流程工业中广泛应用于工程设计、仿真、实时优化和故障诊断。在现代工业 AI 工作流中，机理模型常与数据驱动模型相结合，形成混合（灰盒）建模方法：机理部分负责处理过程的已知结构，数据驱动部分则对不确定性、未测扰动和模型偏差进行修正。TDengine IDMP 通过提供数据驱动组件所需的时序数据基础设施和资产上下文，支撑混合建模工作流——机理模型的输出和数据驱动修正量均可作为计算属性存储，并与实时传感器数据一同进行分析。
