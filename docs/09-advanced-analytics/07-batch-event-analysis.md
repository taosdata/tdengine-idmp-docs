...
用户也可以在事件分析面板操作栏中点击**窗口分析**图标，弹出窗口分析框，用户可以继续配置窗口类型、事件规则等，保存后生成临时事件。

![临时事件生成2](./images/event-trend-adhoc.png)
...
在事件分析面板中，将事件时间范围叠加在工艺参数曲线上，可以直观呈现该事件期间各参数的变化过程。选择多个事件后，可将它们的曲线叠加在同一图表中进行对比，快速识别事件间的工艺差异、参数偏移和异常波动。
...
事件分析面板可以按多种方式展示属性趋势与事件的组合。事件默认是按照较深的底色直接叠加在属性趋势图内部。
...
事件分析面板里集成了时序预测的能力。用户可以点击操作栏的**预测**图标，选择需要进行预测的属性，配置预测算法与参数，一键式生成预测数据。

![预测分析](./images/event-trend-forecast.png)

**缺失值填补**

事件分析面板里集成了缺失值填补的能力。用户可以点击操作栏的**缺失值填补**图标，选择需要进行缺失值填补的时间窗口，配置填补算法与参数，一键式生成缺失值数据，并可以选择重置或保存填补数据。
...
:::note
事件模板的创建与管理（包括自定义属性的定义、命名规则和严重程度配置）在**基础库 → 事件模板**中进行。分析的完整配置说明请参阅[实时智能分析与响应](../07-real-time-analysis/02-creating-analysis.md)章节，事件的完整使用说明请参阅[事件](../06-events/)章节。
:::
...
某汽车零部件厂的注塑车间使用 8 台注塑机生产精密塑料外壳，每批次生产约 1000 件，周期 6～8 小时。注射温度、保压压力和冷却时间是影响外壳尺寸精度和表面质量的关键参数。近期部分批次的不良率明显偏高，工艺团队希望通过事件、批次分析定位问题所在。
...
工艺团队在事件视图中筛选出近 40 个事件，按不良率高低分为两组，在趋势图中叠加对比两组事件的注射温度曲线。
