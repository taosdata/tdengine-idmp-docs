...
TDengine IDMP 中的事件是具有明确开始时间、结束时间和持续时长的离散运营事件，是记录某件事情发生的完整数字档案。

批次分析是工业数据分析中针对离散生产过程的重要方法。**IDMP 将产品批次定义为一类特殊事件**。在 IDMP 平台中，并没有一个独立的批次分析模块，而是将批次作为事件的一种特殊类型，利用强大、灵活的事件分析能力来完成批次的全生命周期管理与深度分析。

IDMP 通过事件分析实现了对批次生产、化工反应、加工制造等场景下全过程数据的对比分析，帮助用户识别影响产品质量的关键工艺因素，发现批次间的规律与异常，为工艺优化和质量管控提供数据支撑。

**分析面板（Analysis Chart）是事件与批次分析的主要入口。** 分析面板是 IDMP 中唯一以独立窗口形式运行的分析面板，用户可以在其中完成事件对比、窗口分析、时序预测、相关分析等多种过程分析方法。本节所述的事件分析与探索、窗口分析等功能均在分析面板的查看页面中进行。
...
窗口分析是在历史数据中交互式搜索有意义时间片段的工具。用户在分析面板的查看页面中按需发起，选择不同的窗口策略对历史数据进行回溯扫描，将符合条件的时间段找出来并叠加显示在当前图表上。

**窗口分析不会创建事件。** 用户定义的窗口以高亮时段的形式叠加在属性趋势曲线上，帮助用户直观观察这些时段内的数据表现。窗口分析是纯粹的可视化探索过程，不会创建系统事件，也不会触发事件的处置流程如告警、提示等。
...
| **是否创建事件** | 是 | 否 |
| **使用入口** | 元素 → 分析 → 配置触发器 | 分析面板查看页面 → 窗口分析图标 |

简言之，实时分析触发器像"监控摄像头"——配置一次后持续运行；窗口分析像"回放录像"——按需在历史数据中搜索特定画面。

### 使用入口

在分析面板查看页面的操作栏中，点击**窗口分析**图标，弹出窗口分析配置框。选择窗口类型、配置参数后，系统对当前时间范围内的历史数据执行窗口搜索，将找到的窗口以高亮时段显示在图表上。

![窗口分析配置](./images/event-trend-adhoc.png)
...
- **异常检测（Anomaly Detection）：** 对目标属性运行异常检测算法，无需人工设定阈值，系统自动识别偏离正常行为的数据段。适合需要 AI 自动发现异常的探索场景
...
找到的窗口在分析面板图表上以高亮时段的形式叠加显示在属性趋势曲线上。用户可以：

- 在同一面板中同时运行多种窗口策略的搜索，对结果进行交叉比对
- 直观观察每个窗口内的数据表现，判断是否需要进一步分析
- 结合事件对比、包络线等分析能力，对感兴趣的窗口做更深入的探索

## 事件分析入口

### 事件查询与过滤

IDMP 提供了强大的事件搜索与过滤能力，帮助用户快速定位需要深入分析的事件。批次事件作为一种特殊的事件类型，可以利用 IDMP 的完整搜索功能体系进行查询。
...
在事件列表中找到目标事件后，点击事件可查看其详细信息（开始/结束时间、持续时长、汇总统计指标、自定义属性等）。从事件详情页面，可以直接跳转到该事件的趋势图分析、多事件对比等深度分析场景。

通过灵活的搜索与过滤，用户可以从海量历史事件中快速定位需要关注的重点事件，为后续的对比分析、质量溯源和工艺优化奠定基础。

### 事件分析与探索

事件生成后，可通过事件视图进行深度分析与探索。IDMP 提供了多种事件对比与可视化方式，帮助用户从不同维度理解事件间的差异与规律。

**批次列表与详情**

在元素的**事件**标签页，可以查看该元素的全部历史事件记录，支持按时间范围、事件类型等条件过滤筛选。点击单个事件，查看该事件的完整信息——开始/结束时间、持续时长、汇总统计指标和自定义属性值。

**临时事件**

除了由系统自动生成或手动录入的事件之外，IDMP 还支持用户直接创建**临时事件**——无需预先配置模板或触发规则，用户可以灵活组合时间范围与属性条件，即时定义并生成所关注的事件。具体而言，用户可以在元素事件列表页，点击 + 后出现弹出窗口**生成测试事件数据**，即可配置个性化事件规则，生成临时事件。

![临时事件生成](./images/adhoc-event.png)
...
多泳道视图将多个事件的曲线分别绘制在独立的子图中，每个属性占据一个"泳道"。这种布局方式避免了曲线重叠造成的视觉混乱，特别适合同时对比大量事件（如 10 个以上）的整体趋势。用户可以快速浏览每个事件的完整过程，识别出明显偏离群体模式的异常批次。

![多泳道分析](./images/event-trend-02.png)

上图展示了多泳道布局下的事件对比。每个属性独占一行，纵向排列，便于逐一检视各事件的过程曲线形态，快速发现异常事件的特征模式。

**事件线分析**

分析面板可以按多种方式展示属性趋势与事件的组合。事件默认是按照较深的底色直接叠加在属性趋势图内部。

用户也可以点击操作栏的**启用事件线模式**，此时事件会以不同颜色的线条形式，展示在属性趋势图的上方，当鼠标挪到事件线的展示位置，系统都会自动提示对应事件的关键信息。这种方式特别适合同时对大量不同类型的事件进行调查分析。

![事件线分析](./images/event-trend-line.png)

**时序预测**

分析面板集成了时序预测的能力。用户可以点击操作栏的**预测**图标，选择需要进行预测的属性，配置预测算法与参数，一键式生成预测数据。关于预测算法的详细说明，请参阅[时序预测](./01-forecasting.md)。

![预测分析](./images/event-trend-forecast.png)

**缺失值填补**

分析面板集成了缺失值填补的能力。用户可以点击操作栏的**缺失值填补**图标，选择需要进行缺失值填补的时间窗口，配置填补算法与参数，一键式生成缺失值数据，并可以选择重置或保存填补数据。关于填补算法的详细说明，请参阅[缺失数据填补](./03-missing-data-imputation.md)。

![缺失值填补](./images/event-trend-imputation.png)

![缺失值填补保存](./images/event-trend-imputation-save.png)
...
上图展示了时间对齐后的事件对比。所有事件的起点对齐到 t=0，横轴表示事件开始后的相对时间。通过这种方式，可以清晰对比各事件在相同相对时间点的参数表现，识别过程执行的一致性。
...
上图展示了时间归一化后的事件对比。所有事件的时间轴被压缩或拉伸到 0%-100% 的统一尺度，横轴表示事件完成进度。通过归一化，可以对比不同时长事件在"前 25%"、"中期 50%"、"收尾阶段"等相对进度点的参数表现，识别过程执行节奏的差异。
...
上图展示了包络线分析。橙色区域表示当前事件的参数波动范围（如均值±2 倍标准差），彩色曲线表示当前事件的上下限参数。当曲线超出包络线范围时，说明该事件在该时间段的参数异常，需要进一步调查原因。包络线为批次质量判定提供了量化的参考基准。
...
某汽车零部件厂的注塑车间使用 8 台注塑机生产精密塑料外壳，每批次生产约 1000 件，周期 6～8 小时。注射温度、保压压力和冷却时间是影响外壳尺寸精度和表面质量的关键参数。近期部分批次的不良率明显偏高，工艺团队希望通过分析面板综合利用窗口分析、事件对比和时序预测等能力定位问题所在。
...
2. **窗口分析探索异常时段：** 在分析面板中打开某台注塑机的注射温度趋势，点击操作栏的**窗口分析**图标，选择**事件窗口**，开始条件设为 `injection_temp < 220`，结束条件设为 `injection_temp > 225`。系统在过去 3 个月数据中找到 18 个温度偏低时段，高亮显示在图表上。工艺团队发现这些时段集中在夜班后半段。
3. **事件对比定位根因：** 在事件视图中筛选出近 40 个批次事件，按不良率高低分为两组，使用趋势叠加对比和时间对齐功能，将两组事件的注射温度曲线叠加在同一图表中。分析确认高不良率批次在生产后半段（约第 5～8 小时）注射温度持续低于 220°C，而低不良率批次全程稳定在 225～235°C。
4. **预测辅助决策：** 在分析面板中对故障注塑机的注射温度启用时序预测，预测曲线显示若不干预，温度将在未来 24 小时内进一步下降，为紧急维修提供了定量依据。

**分析效果**

经排查，原因是料筒加热元件老化导致夜班产能下降后无法维持设定温度。窗口分析帮助团队快速锁定了异常时段的分布规律，事件对比确认了温度偏低与不良率的直接关联，时序预测为维修紧迫性提供了定量依据。更换加热元件并调整工艺参数后，后续 15 个批次的不良率从平均 4.2% 降至 1.1%，注射温度曲线趋于一致。
