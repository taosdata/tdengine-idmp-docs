...
批次分析是工业数据分析中针对离散生产过程的核心方法。IDMP 支持对批次生产、化工反应、加工制造等场景下的全过程数据进行系统化对比分析，帮助用户识别影响产品质量的关键工艺因素。通过发现批次间的规律与异常，为工艺优化和质量管控提供数据依据。
...
除了由系统自动生成或手动录入的批次事件之外，IDMP 还支持直接创建**临时事件**——无需预先配置模板或触发规则，用户可以灵活组合时间范围与属性条件，即时定义并生成所关注的事件。具体而言，用户既可以在图表上直接框选时间范围，也可以在此基础上叠加属性规则（如某参数高于或低于指定阈值、属性值等于某状态等），从而精确圈定满足特定工况条件的时段。

临时事件可以像正式批次事件一样参与分析，与系统中已有的批次记录一起叠加在同一图表中进行对比。这一功能特别适合以下场景：
