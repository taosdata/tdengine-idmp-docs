...
缺失数据填补通过**趋势图**和**分析工作台**在查看模式下的操作栏进行触发和配置。

### 执行填补

**第一步：识别缺失窗口**

打开包含缺失数据属性所在的**趋势图面板**。数据缺口会在图表上以空白区段的形式清晰呈现，便于定位需要处理的时间范围。
...
确认算法配置后，IDMP 调用 TDgpt 对所选时间段进行 AI 估算。填补生成的新数据以**红色高亮**叠加显示在图表上，与原始实测数据在视觉上明确区分，便于在保存前核查估算结果是否合理。此时可点击**重置**撤销本次填补，重新调整时间范围或算法后再次执行。

![填补结果预览（红色高亮为新生成数据）](./images/imputation-03.png)

**第四步：保存并查看最终效果**

确认填补结果满意后，点击**保存**将填补数据写入数据存储，此时趋势图将恢复为完整连续的曲线，原有数据缺口消除。
