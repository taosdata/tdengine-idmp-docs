...
聚类分析通过**散点图面板**查看页面操作栏中的**聚类分析**图标进行访问。

步骤：

1. 打开或创建一个**散点图面板**，配置 X 轴和 Y 轴属性，使面板以散点形式呈现两个属性的联合分布。
2. 在面板的**查看页面**，点击操作栏中的**聚类分析**图标。
...
:::note
当前版本中，聚类分析的使用入口为散点图面板查看页面操作栏的聚类分析图标。未来版本将持续扩展使用方式，包括支持更多算法，以及通过正在研发中的**模型开发管理**模块直接调用聚类分析，构建面向具体工业场景的复杂分析模型。

散点图面板的查看页面操作栏还提供**回归分析**图标，可对散点数据拟合线性、指数或多项式曲线。关于散点图面板的完整配置说明，请参阅[散点图](../04-visualization/02-chart-types/12-scatter-chart.md)章节。
:::
