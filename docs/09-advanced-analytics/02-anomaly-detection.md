...
异常检测本质上是一种**无监督学习**任务。与经典的机器学习算法不同，异常检测无需预先标注样本，而是通过对数据整体分布规律的建模，自动找出那些“与大部分数据不一样”的数据点或数据段。
...
异常检测通过**元素分析**进行配置。导航到元素的**分析**标签页，点击 **+** 创建新分析，在**触发**步骤中选择**异常检测**作为触发类型，即可进入异常检测的配置界面。
