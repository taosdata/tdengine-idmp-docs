...
异常检测是工业数据分析中的核心能力之一。IDMP 支持由 **TDgpt** 驱动的 AI 异常检测，帮助用户从海量时序数据中自动识别偏离正常规律的数据片段，实现异常事件的早期预警与快速定位，防患于未然。
...
在即将发布的版本中，IDMP 将进一步扩展异常检测的边界，支持跨设备和跨指标的异常发现。**多指标异常定位**能够在大量同类时序中自动找出行为最偏离的少数序列，例如从 100 台风机的功率曲线中识别出运行异常的风机；**多元时序异常检测**则基于元素多个属性的联合分布建模，捕获单变量分析无法识别的复合型异常模式，例如温度单独看来正常、但振动与电流同时出现协同偏差的复合故障。
...
配置完成后，系统将持续对所选属性进行实时异常检测。当 TDgpt 识别到异常窗口时，分析自动触发，将异常时段的开始和结束时间写入输出属性；如已启用事件生成，还会同步创建记录该异常时段的事件。

异常事件生成后，可在元素的**事件**标签页查看异常事件列表，或在**事件趋势图**面板中将异常时段叠加显示在原始参数曲线上，直观对比异常窗口内的信号形态与正常段的差异，辅助快速定位根因。

:::note
分析的完整配置说明（包括触发类型参数、计算配置等）请参阅[实时智能分析与响应](../07-real-time-analysis/02-creating-analysis.md)章节。
:::
