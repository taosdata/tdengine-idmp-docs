...
1. 打开**分析面板**，在查看模式下点击**事件、属性列表**图标。系统将弹出汇总窗口，展示当前面板中的属性列表、事件列表以及常用统计指标。

![窗口属性与事件列表](./images/windows-analysis-03.png)

2. 点击操作栏中的**窗口分析**图标，弹出窗口分析配置框。用户可在其中选择窗口类型，并配置对应参数（如条件表达式、状态属性、时间间隔等）。

![窗口分析配置](./images/windows-analysis-01.png)

3. 确认窗口配置后，系统对当前时间范围内的历史数据执行窗口搜索，并将找到的窗口以高亮时段显示在分析面板中。此时，用户可以：
