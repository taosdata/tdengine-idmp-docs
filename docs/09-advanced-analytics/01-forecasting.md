...
| **DeepAR** | 深度学习 | 基于自回归 RNN 的概率预测模型，输出概率分布而非单点预测值 |
...
IDMP 提供两种使用时序预测的方式：一是在元素属性配置中启用预测，预测值将持续计算并自动显示；二是在趋势图和分析面板的查看页面中按需启用预测。
...
配置完成后，打开包含该属性的趋势图面板，预测值将以区别于历史数据的延伸曲线叠加显示在图表末端。

![趋势图中显示的预测结果](./images/forecast-result.png)

### 在面板查看页面操作栏中切换显示

趋势图和分析面板的查看页面操作栏提供**显示预测**图标，用于在不修改属性配置的情况下，快速切换预测叠加层的显示与隐藏。

![趋势图操作栏中的显示预测按钮](./images/forecast-button.png)

点击操作栏中的预测控制图标，可在当前图表上叠加或隐藏预测值，适合在浏览历史数据时快速对比实测值与预测值。
...
1. 在污水处理厂元素的属性列表中，找到 `日进水量` 属性，进入属性详情页并点击**编辑**。
2. 展开**预测配置**，将**预测提供方**设置为 **TDgpt**，算法选择 **Prophet**——进水量不仅有每日和每周的周期规律，还受节假日影响显著，Prophet 对这类含有节假日效应的序列有较强的适应能力；**预测行数**设置为 `24`，覆盖未来 24 小时。
3. 保存后，在厂区监控大屏的趋势图面板中即可看到进水量预测曲线，运营人员每日交班时可直接参考。
