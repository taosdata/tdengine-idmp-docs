...
工业环境中的时序数据难免出现数据缺口——传感器离线、网络中断、硬件故障或数据传输延迟，都可能导致特定时间段内的测量值缺失。IDMP 支持由 **TDgpt** 驱动的 AI 缺失数据填补，利用信号的历史规律对缺口期间的数值进行智能估算，确保下游分析、均值计算和 KPI 统计不因数据缺失而产生偏差。
...
缺失数据填补通过**趋势图**和**事件趋势图**面板的操作栏进行触发和配置，支持查看模式和编辑模式两种场景下使用。

### 在查看模式下执行填补

**第一步：识别缺失窗口**

打开包含缺失数据的属性所在的**趋势图面板**。数据缺口会在图表上以空白区段的形式清晰呈现，便于定位需要处理的时间范围。

![缺失数据填补界面](./images/imputation-01.png)

**第二步：选择时间范围并配置算法**

点击操作栏中的**缺失值填补**按钮，进入填补模式。在图表上**点击并拖动**，框选需要填补的时间范围——建议将选区适当延伸至缺口的前后，以便算法能够参考缺口两侧的实测数据进行估算。松开后，系统弹出**填补算法配置窗口**，可从 Mean、IEM、LSTM、Moment 等算法中选择最适合当前信号特征的选项。

![选择填补时间范围及算法配置](./images/imputation-02.png)

**第三步：预览填补结果**

确认算法配置后，IDMP 调用 TDgpt 对所选时间段进行 AI 估算。填补生成的新数据以**红色高亮**叠加显示在图表上，与原始实测数据在视觉上明确区分，便于在保存前核查估算结果是否合理。此时可点击**重置**撤销本次填补，重新调整时间范围或算法后再次执行；确认结果满意后，点击**保存**将填补数据写入存储。

![填补结果预览（红色高亮为新生成数据）](./images/imputation-03.png)

**第四步：保存并查看最终效果**

保存完成后，填补数据正式写入，趋势图恢复为完整连续的曲线，原有缺口消除。

![保存后的完整曲线](./images/imputation-04.png)

### 在编辑模式下预览填补效果

在面板编辑模式下，操作栏同样提供**缺失值填补**控件，可在面板预览中进入填补模式，对图表效果进行实时预览，辅助判断填补配置是否符合预期，无需切换至查看模式。
...
2. 点击操作栏中的**缺失值填补**按钮，进入填补模式。
3. 在图表上框选数据缺失的时间区域（选区适当延伸至缺口前后），IDMP 调用 TDgpt 分析缺口两侧的流量规律，对缺失时段进行智能估算并填充数值。
4. 填补结果以红色高亮叠加显示，与实测数据可视觉区分；确认估算结果合理后，点击**保存**完成本次填补操作。
