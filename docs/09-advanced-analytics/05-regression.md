...
回归分析通过**散点图面板**查看页面操作栏中的**回归分析**图标进行访问。

步骤：

1. 打开或创建一个**散点图面板**，将 X 轴配置为自变量属性、Y 轴配置为因变量属性，使面板以散点形式呈现两个变量的联合分布。
2. 在面板的**查看页面**，点击操作栏中的**回归分析**图标。
3. 选择回归类型（线性、指数或多项式），如选择多项式回归，还需配置多项式的阶数。
4. IDMP 将自动对当前图表中的数据点拟合回归曲线，并将拟合曲线叠加显示在散点图上。
...
:::note
当前版本中，回归分析的使用入口为散点图面板查看页面操作栏的回归分析图标。未来版本将持续扩展使用方式，包括支持更多算法，以及通过正在研发中的**模型开发管理**模块直接调用回归分析，构建面向具体工业场景的复杂分析模型。

散点图面板的查看页面操作栏还提供**聚类分析**图标，可对散点数据执行聚类分析，识别数据的自然分组结构。关于散点图面板的完整配置说明，请参阅[散点图](../04-visualization/02-chart-types/12-scatter-chart.md)章节。
:::
...
2. 在面板的**查看页面**，点击操作栏中的**回归分析**图标，选择**线性回归**。
