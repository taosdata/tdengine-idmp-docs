...
批次分析是工业数据分析中针对离散生产过程的重要方法。**IDMP 将产品批次定义为一类特殊事件**。平台并未提供独立的批次分析模块，而是将批次作为事件的一种特殊类型，借助灵活的事件分析能力完成批次的全生命周期管理与深度分析。
...
在事件列表中找到目标事件后，点击事件可查看其详细信息（开始/结束时间、持续时长、汇总统计指标、自定义属性等）。从事件详情页面，可以直接跳转到该事件的分析面板进行深度分析。
...
分析面板集成了窗口分析能力，用户可以在事件分析过程中随时切换到窗口分析模式，对历史数据执行交互式搜索。点击操作栏的**窗口分析**图标，选择窗口类型并配置参数，系统即在当前时间范围内扫描数据，将匹配的时间段以高亮窗口叠加显示在图表上。窗口分析的详细原理、窗口类型和使用说明请参阅[窗口分析](./05-window-analysis.md)。
...
用户也可以点击操作栏的**启用事件线模式**，此时事件会以不同颜色的线条形式，展示在属性趋势图的上方。当鼠标悬停在事件线上时，系统将自动提示对应事件的关键信息。这种方式特别适合同时对大量不同类型的事件进行调查分析。

![事件线分析](./images/event-batch-line.png)
