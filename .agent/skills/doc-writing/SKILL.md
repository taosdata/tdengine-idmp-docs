...
优先使用 Docusaurus 内置的 callout/admonition 语法（`:::` 围栏），**不要**使用 Markdown 引用块 `>` 来表达提示、说明、警告等信息。内置 callout 渲染为醒目的提示框，且语义清晰、样式统一。
...
- 需要强调的说明性内容，一律使用 `:::info` / `:::note` / `:::tip` / `:::warning` / `:::danger` 之一，而非 `> 引用块`。
- `:::info` 用于中性补充说明；`:::tip` 用于建议/技巧；`:::warning` 用于需注意的警告；`:::danger` 用于危险操作。
- callout 内容前后各空一行，`:::` 开闭标签各占一行。
- 可选的 callout 标题写在同一行冒号后：`:::info 标题文字`。
