...
def main() -> int:
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Show what would happen without writing any files; always exits 0",
    )
    parser.add_argument(
        "--bootstrap",
        action="store_true",
    ...
    # --- CLI contract: enforce forbidden flag combinations ---
    forbidden = [
        (args.force and args.check, "--force and --check are mutually exclusive"),
        (args.force and args.dry_run, "--force and --dry-run are mutually exclusive"),
        (args.check and args.dry_run, "--check and --dry-run are mutually exclusive"),
        (args.bootstrap and args.check, "--bootstrap and --check are mutually exclusive"),
        (args.bootstrap and args.dry_run, "--bootstrap and --dry-run are mutually exclusive"),
        (args.bootstrap and args.force, "--bootstrap and --force are mutually exclusive"),
    ]
    for condition, message in forbidden:
        if condition:
    ...
    # Normal mode: write prompt, write needs-extraction.yaml, and report
    ...
    print(f"Written: {NEEDS_EXTRACTION_FILE}")
